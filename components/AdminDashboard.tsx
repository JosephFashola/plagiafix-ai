...
import { 
  ...
  WifiOff, Copy, AlertCircle, Zap, History, TrendingUp, CheckCircle2, XCircle
} from 'lucide-react';
import { 
...
const StatCard: React.FC<{ 
  ...
  <div className="bg-[#111827]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-2xl relative group overflow-hidden transition-all hover:border-indigo-500/40">
    ...
    <h4 className="text-2xl sm:text-3xl font-black text-white mt-1 tracking-tighter">
...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-slate-200 p-4 sm:p-6 md:p-10 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      <div className="max-w-[1700px] mx-auto space-y-6 sm:space-y-8">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sm:gap-8 bg-[#111827]/50 backdrop-blur-xl p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-white/5 shadow-2xl">
          <div className="flex items-center gap-4 sm:gap-6">
            <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl ${health?.db === 'OK' ? 'bg-indigo-600' : 'bg-rose-600 animate-pulse'}`}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            <div className="flex bg-slate-900 p-1 rounded-xl sm:rounded-2xl border border-white/5 overflow-x-auto scrollbar-none">
              {[
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${activeViewVal === v.id ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-500 hover:text-white'}`}
                >
                  {v.icon}
                  {v.label}
                </button>
            ...
            <button onClick={refreshData} className="p-3 bg-white/5 rounded-xl sm:rounded-2xl border border-white/10 text-white hover:bg-white/10 transition-all flex justify-center">
        ...
        {/* Daily Health Audit Panel */}
        <div className="bg-gradient-to-r from-indigo-900/20 to-purple-900/20 border border-indigo-500/20 rounded-[1.5rem] p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
           <div className="flex items-center gap-4">
              <div className="p-3 bg-indigo-500/20 rounded-xl">
                 <HeartPulse className="w-6 h-6 text-indigo-400" />
              </div>
              <div>
                 <h3 className="text-sm font-black text-white uppercase tracking-widest">Daily Health Audit</h3>
                 <p className="text-[10px] text-slate-400 font-medium">Automated system diagnostics and latency verification.</p>
              </div>
           </div>
           <div className="flex gap-4 sm:gap-8">
              <div className="flex flex-col items-center">
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Gemini Latency</span>
                 <div className="flex items-center gap-2">
                    {health?.gemini === 'OK' ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <XCircle className="w-4 h-4 text-rose-500" />}
                    <span className="text-xl font-black text-white">{health?.geminiLatency || 0}ms</span>
                 </div>
              </div>
              <div className="w-px h-10 bg-white/5 hidden md:block"></div>
              <div className="flex flex-col items-center">
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">DB Latency</span>
                 <div className="flex items-center gap-2">
                    {health?.db === 'OK' ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <XCircle className="w-4 h-4 text-rose-500" />}
                    <span className="text-xl font-black text-white">{health?.dbLatency || 0}ms</span>
                 </div>
              </div>
              <div className="w-px h-10 bg-white/5 hidden md:block"></div>
              <div className="flex flex-col items-center">
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Session Uptime</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          <div className="lg:col-span-8 space-y-6 sm:space-y-8">
            {activeViewVal === 'overview' && (
              <div className="bg-[#111827]/80 backdrop-blur-xl border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 min-h-[400px] sm:h-[550px] shadow-2xl animate-in fade-in zoom-in duration-500">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-10">
                  ...
                  <select value={range} onChange={(e) => setRange(e.target.value as any)} className="w-full sm:w-auto bg-slate-900 border border-white/5 rounded-xl px-4 py-2 text-[10px] font-black uppercase text-slate-400 outline-none hover:border-indigo-500 transition-all">
            ...
            {activeViewVal === 'health' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 animate-in slide-in-from-right duration-500">
                <div className="bg-[#111827]/80 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 space-y-6 shadow-2xl">
                  <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
                    <Signal className="w-5 h-5 text-indigo-400" /> Sync Health
                  </h3>
                  <div className="p-4 sm:p-6 bg-slate-900 rounded-2xl border border-white/5 space-y-4">
                  ...
                  <div className="p-4 sm:p-6 bg-slate-900 rounded-2xl border border-white/5">
                    <h4 className="text-[10px] font-black text-slate-500 uppercase mb-2">Supabase Response</h4>
                    <div className="text-[9px] sm:text-[10px] font-mono text-rose-400 bg-rose-500/5 p-4 rounded-xl border border-rose-500/10 overflow-hidden break-words">
                ...
                <div className="bg-[#111827]/80 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 space-y-6 shadow-2xl">
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto scrollbar-none font-mono">
                    {rawSample.data.map((s, i) => (
                      <div key={i} className="p-3 sm:p-4 bg-slate-900 border border-white/5 rounded-xl text-[9px] sm:text-[10px]">
            ...
            {activeViewVal === 'geography' && (
              <div className="space-y-6 sm:space-y-8 animate-in slide-in-from-left duration-500">
                <div className="bg-[#111827]/80 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 shadow-2xl">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
                      ...
                      {countryData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={countryData} layout="vertical">
                            ...
                            <YAxis dataKey="name" type="category" stroke="#4b5563" fontSize={9} width={80} tickFormatter={(val) => COUNTRY_NAMES[val] || val} />
                            <Tooltip 
                        ...
                        <div className="h-full flex items-center justify-center text-slate-600 text-[9px] sm:text-[10px] font-black uppercase tracking-widest italic opacity-30">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      {countryData.slice(0, 4).map((c, i) => (
                        <div key={i} className="p-4 sm:p-6 bg-slate-900 rounded-2xl sm:rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center transition-all hover:bg-white/5">
                           <span className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{COUNTRY_NAMES[c.name] || c.name}</span>
                           <h4 className="text-2xl sm:text-3xl font-black text-white">{c.value}</h4>
                           <div className="w-6 sm:w-8 h-1 bg-indigo-600 rounded-full mt-2"></div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-[#111827]/80 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 shadow-2xl">
                   <h3 className="text-[10px] sm:text-xs font-black text-white uppercase tracking-widest mb-6 flex items-center gap-2">
                   ...
                   <div className="space-y-3">
                     {logs.filter(l => l.type === 'VISIT').slice(0, 10).map((log, i) => {
                       ...
                       return (
                         <div key={i} className="flex items-center justify-between p-3 sm:p-4 bg-slate-900/50 rounded-xl sm:rounded-2xl border border-white/5 hover:border-white/10 transition-all">
                            <div className="flex items-center gap-3 sm:gap-4">
                               <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-600/20 rounded-lg sm:rounded-xl flex items-center justify-center text-indigo-400 font-black text-[10px] sm:text-xs">
                                 {code}
                               </div>
                               <div>
                                 <h4 className="text-[10px] sm:text-[11px] font-black text-white uppercase tracking-widest">{COUNTRY_NAMES[code] || 'Unknown Location'}</h4>
                                 <p className="text-[9px] sm:text-[10px] text-slate-500 font-mono">Session Origin • Forensic Verified</p>
                               </div>
                            </div>
                            <span className="text-[9px] sm:text-[10px] font-black text-slate-600 uppercase">
                     ...
                     {logs.filter(l => l.type === 'VISIT').length === 0 && (
                        <div className="py-10 sm:py-20 text-center opacity-20 italic text-sm">No recorded geo-hits in session cache.</div>
                     )}
                   </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 sm:gap-8">
            <div className="bg-[#111827]/80 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] flex flex-col h-full overflow-hidden shadow-2xl">
              <div className="p-6 sm:p-8 border-b border-white/5 bg-white/5 flex items-center justify-between">
                <h3 className="text-[10px] sm:text-xs font-black text-white uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 max-h-[500px] sm:max-h-[850px] scrollbar-none">
                {logs.map((log, i) => (
                  <div key={i} className="p-4 sm:p-5 bg-slate-900/30 border border-white/5 rounded-2xl sm:rounded-3xl transition-all hover:bg-white/5 group">
                    <div className="flex justify-between items-center mb-2 sm:mb-3">
                      <div className="flex items-center gap-2">
                        <span className={`text-[7px] sm:text-[8px] font-black uppercase px-2 py-0.5 sm:py-1 rounded ${log.type === 'SCAN' ? 'bg-indigo-500/10 text-indigo-400' : log.type === 'FIX' ? 'bg-purple-500/10 text-purple-400' : log.type === 'VISIT' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                          {log.type}
                        </span>
                        {log.type === 'VISIT' && log.details.match(/\[([A-Z]{2})\]/) && (
                          <span className="text-[7px] sm:text-[8px] font-black text-slate-500 uppercase tracking-widest border border-white/5 px-2 py-0.5 sm:py-1 rounded">
                            {log.details.match(/\[([A-Z]{2})\]/)?.[1]}
                          </span>
                        )}
                      </div>
                      <span className="text-[7px] sm:text-[8px] font-mono text-slate-600 group-hover:text-slate-400 transition-colors">
                        {new Date(log.timestamp).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                    <p className="text-[10px] sm:text-[11px] text-slate-400 leading-relaxed line-clamp-2">{log.details}</p>
                  </div>
                ))}
              </div>
            </div>
